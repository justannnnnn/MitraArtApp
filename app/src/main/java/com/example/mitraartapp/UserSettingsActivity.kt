...
class UserSettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        photoImageView.setShapeAppearanceModel(
            photoImageView.getShapeAppearanceModel().toBuilder()
        ...
        photoImageView.setOnClickListener {
            ...
            view = layoutInflater.inflate(R.layout.button_photo_dialog, null)
        ...
        // Email TextView
        val emailText = findViewById<TextView>(R.id.emailTextViewClickable)
        emailText.text = dbHandler.getEmail()
        emailText.setOnClickListener {
            val dialog = BottomSheetDialog(this)
            val viewEmail = layoutInflater.inflate(R.layout.button_email_change_dialog, null)


            val newEmailTextField =
                viewEmail.findViewById<TextInputEditText>(R.id.newEmail_TextInput)
            val validation = ValidationHelper()
            newEmailTextField.doAfterTextChanged {
                var email = newEmailTextField.text
                if (!validation.isEmail(email)!!) {
                    newEmailTextField.setError("Invalid format for email")
                } else newEmailTextField.error = null
            }


            val sendLinkButton = viewEmail.findViewById<Button>(R.id.send_link_button)
            sendLinkButton.setOnClickListener {
                if (newEmailTextField.error == null) {
                    var email = newEmailTextField.text.toString()
                    ...
                    mAuth!!.signInWithEmailAndPassword(email, "password")
                            .addOnCompleteListener(this) { task ->
                                if (task.isSuccessful) {
                                    ...
                                    verifyEmail()
                                    while (mAuth!!.currentUser?.isEmailVerified() == false){}
                                    Toast.makeText(
                                        this,
                                        "User is verified...",
                                        Toast.LENGTH_SHORT
        ...
        override fun onResume() {
            ...
            if (hasPhoto) {
                ...
                val view = layoutInflater.inflate(R.layout.button_photo_dialog, null)
        ...
        fun downloadAvatar(photoFileStr: String) {
            ...
            if ("com.google.android.apps.photos.contentprovider" in photoFileString) {
                ...
                val resolver = contentResolver
                val cursor = Uri.parse(photoFileString)
                    .let { it1 -> resolver.query(it1, null, null, null, null) }
                val dataIndex = cursor!!.getColumnIndex(MediaStore.Images.ImageColumns.DATA)
                cursor.moveToFirst()
                val imagePath = cursor.getString(dataIndex)
                val myBitmap = BitmapFactory.decodeFile(imagePath)
                ...
                val myBitmap = MediaStore.Images.Media.getBitmap(
                    this.contentResolver,
                    Uri.fromFile(File(photoFileString))
                )
                photoImageView.setImageBitmap(Bitmap.createScaledBitmap(myBitmap, 150, 150, false))
            }
        }
        ...
        /*private fun verifyEmail() {
            ...
            val actionCodeSettings = ActionCodeSettings.newBuilder()
                .setUrl("https://art.viomitra.ru/")
                .build()
            mUser!!.sendEmailVerification(actionCodeSettings)
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
